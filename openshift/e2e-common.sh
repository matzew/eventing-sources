...
function install_knative_kafka_channel_tls(){
  ...
  RELEASE_YAML="openshift/release/knative-eventing-kafka-channel-ci-tls.yaml"
  ...
  cat ${RELEASE_YAML} \
  | sed "s/REPLACE_WITH_CLUSTER_URL/${KAFKA_TLS_CLUSTER_URL}/" \
  | oc apply --filename -
...
function run_e2e_auth_tests(){
  header "Testing the consolidated channel with TLS/SASL"
